---
name: ambition
description: "Use this skill when the user asks to pressure test whether this goal is ambitious enough, create an Ambition Audit or Goal Stretch Brief, audit an existing draft, or makes a near-miss request that would invent evidence or overstep human authority. It produces a concrete Ambition Audit or Goal Stretch Brief with facts, inferences, gaps, owners, dates, measures, decisions, and failure modes explicit."
...
This skill tests one supplied goal for meaningful stretch, constraint removal, evidence, and risk. It does not design a 10x solution, set a performance rating, or choose a goal for the user.

## Artifact contract

| Mode | Input | Output |
|---|---|---|
| Build | Supplied facts, constraints, evidence, owners, dates, and decisions | Ambition Audit or Goal Stretch Brief |
| Audit | Existing artifact and any supplied standard | Ambition Audit with prioritized repairs |
...
`think-big`, `goals`, `10-universal-business-objectives` may accept a handoff when installed. If absent, finish this artifact and label the optional handoff. Do not absorb the related skill's purpose.

## Input contract

- goal and intended beneficiary
- baseline and evidence source
- time horizon and owner
...
Use `assets/ambition-audit-template.md`. Include:

- Goal frame
- Baseline and evidence
- Stretch dimensions
- Constraint tests
- Risk and support plan
- Owner commitment
...
- Do not invent a baseline, target, growth rate, capacity, upside, cost, or probability.
- Do not evaluate a person's ambition, potential, personality, promotion readiness, or performance.
- Do not turn a stretch brief into a commitment until the authorized owner selects it.
...
**Hypothetical request:** Audit this hypothetical goal: reduce intake rework from the supplied July baseline of 7 returned requests out of 42 to a target the owner has not selected by December 31. Owner: Intake Lead. Constraint: source access depends on another team.

The first draft uses only the supplied facts and reserves approval or employment decisions for authorized humans.

## Reference

Read `references/stretch-standard.md` for evidence checks, failure modes, and the distinct execution boundary.
